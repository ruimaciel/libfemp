#include <libfemp/Analysis.hpp>  // for the error codes
...
BaseElement::getStiffnessMatrix(fem::Model& model) {
  ...
  const fem::Material material =
      model.getMaterialByIndex(this->getMaterialRef());
  ...
  for (auto quadrature_point : this->getStiffnessQuadratureRule()) {
    ...
    for (unsigned int n = 0; n < nnodes; n++) {
      ...
      double dNdx = invJ(0, 0) * dNdcsi[n] + invJ(0, 1) * dNdeta[n] +
                    invJ(0, 2) * dNdzeta[n];
      double dNdy = invJ(1, 0) * dNdcsi[n] + invJ(1, 1) * dNdeta[n] +
                    invJ(1, 2) * dNdzeta[n];
      double dNdz = invJ(2, 0) * dNdcsi[n] + invJ(2, 1) * dNdeta[n] +
                    invJ(2, 2) * dNdzeta[n];
...
Eigen::Matrix<double, 6, 6> BaseElement::generateConstitutiveRelationsMatrix(
