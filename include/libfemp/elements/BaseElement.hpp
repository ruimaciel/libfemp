...
/**
 * Base class for all classes which are used to describe specific FEM element
 * types
 */
class BaseElement {
 public:
 ...
 public:  // WARNING: deprecated. to be removed.
  std::map<int, std::vector<boost::tuple<fem::Point3D, double>>>
      ipwpl;  // integration points/weights pair list
 ...
 public:
  ...
  /**
   * return a vector with the value of the derivative of each basis function wrt
   * csi evaluated at a point
   * @param p a point
   ...
   */
  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> getStiffnessMatrix(
      fem::Model& model);

  /**
   * Returns a vector with the reference IDs of all nodes associated with the
   * element
   */
 ...
 public:  // WARNING: BaseElement member functions are deprecated
   ...
   */
  virtual std::vector<boost::tuple<fem::Point3D, double>>
  getStiffnessQuadratureRule() = 0;
  virtual std::vector<boost::tuple<fem::Point3D, double>>
  getDomainQuadratureRule() = 0;

 protected:
  Eigen::Matrix<double, 6, 6> generateConstitutiveRelationsMatrix(
