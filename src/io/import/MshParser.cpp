...
enum MshParser::Error::Type MshParser::parse(std::istream& file,
                                             fem::Model& model) {
  ...
  while (!ss.empty()) {
    if (terminal_token == ss.top()) {
      ...
      ss.pop();
      parser_rule =
          table[(enum NonTerminalToken)nonterminal_token][terminal_token];
      switch (parser_rule) {
        case PR_START:
          ss.push(TT_EOF);
          ss.push(NT_DOCUMENT);
        ...
        case PR_E_ELEMENT_TYPE_SET_FORCE:
          ...
          if (surface_load_operator) {
            ...
            fem::SurfaceLoad* load =
                fem::SurfaceLoad::makeSurfaceLoad(&element);
        ...
        case PR_ELEMENT_FOLLOW_3:
          ...
          this->error.message =
              ...
              boost::lexical_cast<std::string>(error.line_number);
          return this->error.code;
          break;
        ...
        default:
          this->error.code = Error::ERR_UNKNOWN;
          this->error.message =
              "Unknown error while parsing line " +
              boost::lexical_cast<std::string>(error.line_number);
          return this->error.code;
          break;
      }
...
void MshParser::setParserTable() {
  ...
  table[NT_MF_VERSION_NUMBER][TT_UNKNOWN_TOKEN] =
      PR_MF_VERSION_NUMBER_ERROR;  // error just in case
  ...
  table[NT_N_NODE_DEFINITION_FOLLOW][TT_CLOSE_NODES] =
  ...
  table[NT_E_ELEMENT_TYPE][TT_ELEMENT_TETRAHEDRON10] =
  ...
  table[NT_E_ELEMENT_DEFINITION_FOLLOW][TT_INDEX] =
      PR_E_ELEMENT_DEFINITION_FOLLOW_1;
  table[NT_E_ELEMENT_DEFINITION_FOLLOW][TT_CLOSE_ELEMENTS] =
...
enum MshParser::Error::Type MshParser::operator()(std::istream& file,
                                                  fem::Model& model) {
  return parse(file, model);
}

void MshParser::setSurfaceLoadOperator(
