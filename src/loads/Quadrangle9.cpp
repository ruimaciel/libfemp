...
Quadrangle9::Quadrangle9() {
  ...
  m_domain_quadrature_rule = std::unique_ptr<QuadrangleCartesianProduct>(
      new QuadrangleCartesianProduct(rule_x, rule_y));
}

std::vector<double> Quadrangle9::getN(const Point3D& p) const {
...
std::vector<double> Quadrangle9::getdNdcsi(const Point3D& p) const {
  double csi = p.x();
  double eta = p.y();
  ...
  dNdcsi[5] =
      (csi + 1) * (1 - eta) * (eta + 1) / 2 + csi * (1 - eta) * (eta + 1) / 2;
  dNdcsi[6] = (1 - csi) * eta * (eta + 1) / 2 - (csi + 1) * eta * (eta + 1) / 2;
  dNdcsi[7] =
      csi * (1 - eta) * (eta + 1) / 2 + (csi - 1) * (1 - eta) * (eta + 1) / 2;
  dNdcsi[8] =
      (1 - csi) * (1 - eta) * (eta + 1) - (csi + 1) * (1 - eta) * (eta + 1);

  return dNdcsi;
}

std::vector<double> Quadrangle9::getdNdeta(const Point3D& p) const {
  double csi = p.x();
  double eta = p.y();
  ...
  dNdeta[4] =
      (1 - csi) * (csi + 1) * eta / 2 + (1 - csi) * (csi + 1) * (eta - 1) / 2;
  dNdeta[5] = csi * (csi + 1) * (1 - eta) / 2 - csi * (csi + 1) * (eta + 1) / 2;
  dNdeta[6] =
      (1 - csi) * (csi + 1) * (eta + 1) / 2 + (1 - csi) * (csi + 1) * eta / 2;
  dNdeta[7] = (csi - 1) * csi * (1 - eta) / 2 - (csi - 1) * csi * (eta + 1) / 2;
  dNdeta[8] =
      (1 - csi) * (csi + 1) * (1 - eta) - (1 - csi) * (csi + 1) * (eta + 1);

  return dNdeta;
}

std::vector<fem::Point3D> Quadrangle9::getLocalCoordinates() const {
  return {
      Point3D(-0.5, -0.5, 0), Point3D(0.5, -0.5, 0), Point3D(0.5, 0.5, 0),
      Point3D(-0.5, 0.5, 0),  Point3D(0.0, -0.5, 0), Point3D(0.5, 0.0, 0),
      Point3D(0.0, 0.5, 0),   Point3D(-0.5, 0.0, 0), Point3D(0.0, 0.0, 0),
  };
}
