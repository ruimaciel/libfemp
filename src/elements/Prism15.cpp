...
std::vector<double> Prism15::getdNdcsi(const Point3D& p) {
  ...
  dNdcsi[0] =
      -(zeta - 1) * (zeta + 2 * (eta + csi)) / 2 - (eta + csi - 1) * (zeta - 1);
  dNdcsi[1] = (zeta - 1) * (zeta - 2 * csi + 2) / 2 - csi * (zeta - 1);
  dNdcsi[2] = 0;
  dNdcsi[3] =
      (eta + csi - 1) * (zeta + 1) - (zeta + 1) * (zeta - 2 * (eta + csi)) / 2;
