...
/**
Opengl: a storage policy class intended to implement the interfaces specific to
the dense, dynamically resizeable matrix to be used in the Matrix host class.
**/
...
Opengl<scalar>::Opengl(const size_t rows, const size_t columns) {
  // the default arguments for class Matrix's constructors set the size as 1.
  // This class ignores that.
  for (int i = 0; i < 16; i++)
    data.data[i] = 0;
}
