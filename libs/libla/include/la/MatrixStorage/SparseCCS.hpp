...
This class implements the interface for the sparse Compressed Row Storage (CRS)
...
void SparseCCS<scalar>::clear() {
  // TODO properly clear the sparse matrix instead of just zeroing the existing
  // values
  using namespace std;
  for (typename vector<scalar>::iterator i = data.values.begin();
       i != data.values.end(); i++) {
    *i = 0;
  }
}
...
scalar SparseCCS<scalar>::value(const size_t row, const size_t column) {
  ...
  for (i = data.column_pointer[column]; i < data.column_pointer[column + 1];
       i++) {
...
scalar& SparseCCS<scalar>::operator()(const size_t row, const size_t column) {
  ...
  // the first element in this column has a greater row index than the one being
  // referenced
  if (row < (size_t)data.row_index[data.column_pointer[column]]) {
  ...
  // the first element in this row has an inferior column index.  Search for the
  // first superior instance, insert a new element and reference it
  long int j;
  for (j = data.column_pointer[column]; j < data.column_pointer[column + 1];
       j++) {
...
void SparseCCS<scalar>::resize(const size_t rows, const size_t columns) {
  ...
  // TODO find a better way to initialize a CRS matrix than setting zeros at the
  // diagonal

  data.values.resize(columns);
  data.row_index.resize(columns);
  data.column_pointer.resize(columns +
                             1);  // to account to the after-last element guard
