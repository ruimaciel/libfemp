...
Prism18::Prism18() : PrismFamily() {
  ...
  m_stiffness_quadrature_rule = std::unique_ptr<PrismCartesianProduct>(
      new PrismCartesianProduct(rule_xy, rule_z));

  rule_xy = new Triangle6();
  rule_z = new GaussLegendre3();
  m_domain_quadrature_rule = std::unique_ptr<PrismCartesianProduct>(
      new PrismCartesianProduct(rule_xy, rule_z));
}

std::vector<double> Prism18::getN(const Point3D& p) {
  ...
  N[0] =
      ((-1 + csi + eta) * (-1 + 2 * csi + 2 * eta) * (-1 + zeta) * zeta) / 2.;
...
std::vector<double> Prism18::getdNdcsi(const Point3D& p) {
  double csi = p.x();
  double eta = p.y();
  double zeta = p.z();
  ...
  dNdcsi[0] = (2 * eta + 2 * csi - 1) * (zeta - 1) * zeta / 2 +
              (eta + csi - 1) * (zeta - 1) * zeta;
  dNdcsi[1] = (2 * csi - 1) * (zeta - 1) * zeta / 2 + csi * (zeta - 1) * zeta;
  dNdcsi[2] = 0;
  dNdcsi[3] = (2 * eta + 2 * csi - 1) * zeta * (zeta + 1) / 2 +
              (eta + csi - 1) * zeta * (zeta + 1);
  dNdcsi[4] = (2 * csi - 1) * zeta * (zeta + 1) / 2 + csi * zeta * (zeta + 1);
  dNdcsi[5] = 0;
  dNdcsi[6] =
      -2 * (eta + csi - 1) * (zeta - 1) * zeta - 2 * csi * (zeta - 1) * zeta;
  dNdcsi[7] = -2 * eta * (zeta - 1) * zeta;
  dNdcsi[8] = -(2 * eta + 2 * csi - 1) * (zeta * zeta - 1) -
              2 * (eta + csi - 1) * (zeta * zeta - 1);
  dNdcsi[9] = 2 * eta * (zeta - 1) * zeta;
  dNdcsi[10] = -(2 * csi - 1) * (zeta * zeta - 1) - 2 * csi * (zeta * zeta - 1);
  dNdcsi[11] = 0;
  dNdcsi[12] =
      -2 * (eta + csi - 1) * zeta * (zeta + 1) - 2 * csi * zeta * (zeta + 1);
  dNdcsi[13] = -2 * eta * zeta * (zeta + 1);
  dNdcsi[14] = 2 * eta * zeta * (zeta + 1);
  dNdcsi[15] =
      4 * (eta + csi - 1) * (zeta * zeta - 1) + 4 * csi * (zeta * zeta - 1);
...
std::vector<double> Prism18::getdNdeta(const Point3D& p) {
  double csi = p.x();
  double eta = p.y();
  double zeta = p.z();
  ...
  dNdeta[0] = (2 * eta + 2 * csi - 1) * (zeta - 1) * zeta / 2 +
              (eta + csi - 1) * (zeta - 1) * zeta;
  dNdeta[1] = 0;
  dNdeta[2] = (2 * eta - 1) * (zeta - 1) * zeta / 2 + eta * (zeta - 1) * zeta;
  dNdeta[3] = (2 * eta + 2 * csi - 1) * zeta * (zeta + 1) / 2 +
              (eta + csi - 1) * zeta * (zeta + 1);
  dNdeta[4] = 0;
  dNdeta[5] = (2 * eta - 1) * zeta * (zeta + 1) / 2 + eta * zeta * (zeta + 1);
  dNdeta[6] = -2 * csi * (zeta - 1) * zeta;
  dNdeta[7] =
      -2 * (eta + csi - 1) * (zeta - 1) * zeta - 2 * eta * (zeta - 1) * zeta;
  dNdeta[8] = -(2 * eta + 2 * csi - 1) * (zeta * zeta - 1) -
              2 * (eta + csi - 1) * (zeta * zeta - 1);
  ...
  dNdeta[13] =
      -2 * (eta + csi - 1) * zeta * (zeta + 1) - 2 * eta * zeta * (zeta + 1);
  dNdeta[14] = 2 * csi * zeta * (zeta + 1);
  dNdeta[15] = 4 * csi * (zeta * zeta - 1);
  dNdeta[16] =
      4 * (eta + csi - 1) * (zeta * zeta - 1) + 4 * eta * (zeta * zeta - 1);
  dNdeta[17] = -4 * csi * (zeta * zeta - 1);

  return dNdeta;
}

std::vector<double> Prism18::getdNdzeta(const Point3D& p) {
  double csi = p.x();
  double eta = p.y();
  double zeta = p.z();
  ...
  dNdzeta[0] = (eta + csi - 1) * (2 * eta + 2 * csi - 1) * zeta / 2 +
               (eta + csi - 1) * (2 * eta + 2 * csi - 1) * (zeta - 1) / 2;
  dNdzeta[1] =
      csi * (2 * csi - 1) * zeta / 2 + csi * (2 * csi - 1) * (zeta - 1) / 2;
  dNdzeta[2] =
      eta * (2 * eta - 1) * zeta / 2 + eta * (2 * eta - 1) * (zeta - 1) / 2;
  dNdzeta[3] = (eta + csi - 1) * (2 * eta + 2 * csi - 1) * (zeta + 1) / 2 +
               (eta + csi - 1) * (2 * eta + 2 * csi - 1) * zeta / 2;
  dNdzeta[4] =
      csi * (2 * csi - 1) * (zeta + 1) / 2 + csi * (2 * csi - 1) * zeta / 2;
  dNdzeta[5] =
      eta * (2 * eta - 1) * (zeta + 1) / 2 + eta * (2 * eta - 1) * zeta / 2;
  dNdzeta[6] = -2 * csi * (eta + csi - 1) * zeta -
               2 * csi * (eta + csi - 1) * (zeta - 1);
  dNdzeta[7] = -2 * eta * (eta + csi - 1) * zeta -
               2 * eta * (eta + csi - 1) * (zeta - 1);
  ...
  dNdzeta[12] = -2 * csi * (eta + csi - 1) * (zeta + 1) -
                2 * csi * (eta + csi - 1) * zeta;
  dNdzeta[13] = -2 * eta * (eta + csi - 1) * (zeta + 1) -
                2 * eta * (eta + csi - 1) * zeta;
