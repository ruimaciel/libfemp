...
#include <libfemp/elements/all.hpp>  // remove individual elements after elements are refactored
...
class Analysis {
 ...
 public:
   ...
   * @param result	a pointer to a AnalysisResult object, where the run
   * results will be stored
   * @return an error
   */
  enum Error buildEquation(Model& Model,
   ...
   * @return an error
   */
  virtual enum Error run(Model& model,
                         LoadPattern& lp,
                         AnalysisResult& result,
                         ProgressIndicatorStrategy& progress) = 0;
   ...
   * @param progress
   */
  enum Error generateGlobalStiffnessMatrix(Model& model,
                                           AnalysisResult& result,
                                           ProgressIndicatorStrategy& progress);
   ...
   * @param progress
   */
  enum Error generateGlobalDomainForceVector(
      Model& model,
      const LoadPattern& lp,
      AnalysisResult& result,
      ProgressIndicatorStrategy& progress);
   ...
   * @param progress
   */
  enum Error generateGlobalSurfaceForceVector(
      Model& model,
      const LoadPattern& lp,
      AnalysisResult& result,
      ProgressIndicatorStrategy& progress);
   ...
   * @param progress
   */
  enum Error generateGlobalPointForceVector(
      Model& model,
      const LoadPattern& lp,
      AnalysisResult& result,
      ProgressIndicatorStrategy& progress);
 ...
 protected:
   ...
   * @param result	a fem::AnalysisResult object, where the information will
   * be stored
   */
   ...
   */
  void addElementaryStiffnessToGlobal(
