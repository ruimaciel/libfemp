...
void assign(Matrix<scalar, MatrixStoragePolicy>& lhs,
            Matrix<scalar, SparseDOK>& rhs) {
  using namespace std;

  lhs.resize(rhs.rows(), rhs.columns());

  for (typename map<size_t, scalar>::iterator i = rhs.data.data.begin();
       i != rhs.data.data.end(); i++) {
...
void assign(Matrix<scalar, SparseCCS>& lhs, Matrix<scalar, SparseDOK>& rhs) {
  ...
  lhs.data.column_pointer.clear(),
      lhs.data.column_pointer.reserve(rhs.columns());
...
scalar ppmvm(Vector<scalar, VectorStoragePolicy>& lhsv,
             Matrix<scalar, MatrixStoragePolicy>& m,
             Vector<scalar, VectorStoragePolicy>& rhsv) {
...
#define rcol (c->first / m.data.t_rows)
  for (typename std::map<size_t, scalar>::iterator c = m.data.data.begin();
       c != m.data.data.end(); c++) {
    if (i != rrow) {
      out += lhsv.value(i) * v;
      v = 0;
