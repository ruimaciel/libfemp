...
#include "MatrixStorage/Dense.hpp"
#include "MatrixStorage/Diagonal.hpp"
#include "MatrixStorage/LowerTriangular.hpp"
#include "MatrixStorage/SparseCCS.hpp"
#include "MatrixStorage/SparseCRS.hpp"
#include "MatrixStorage/SparseDOK.hpp"

namespace lalib {

template <typename scalar,
          template <typename> class MatrixStoragePolicy = DenseMatrix>
class Matrix : public MatrixStoragePolicy<scalar> {
 public:
  ...
  /**
                          Returns a reference to this matrix' element in
     [row,column]
                          **/
...
Matrix<scalar, MatrixStoragePolicy>::Matrix(const size_t rows,
                                            const size_t columns)
    : MatrixStoragePolicy<scalar>(rows, columns) {}

template <typename scalar, template <typename> class MatrixStoragePolicy>
Matrix<scalar, MatrixStoragePolicy>::Matrix(
    Matrix<scalar, MatrixStoragePolicy>& other)
    : MatrixStoragePolicy<scalar>(other) {
...
template <typename scalar, template <typename> class MatrixStoragePolicy>
scalar Matrix<scalar, MatrixStoragePolicy>::value(const size_t row,
                                                  const size_t column) {
  return MatrixStoragePolicy<scalar>::value(row, column);
}

template <typename scalar, template <typename> class MatrixStoragePolicy>
scalar& Matrix<scalar, MatrixStoragePolicy>::operator()(const size_t row,
                                                        const size_t column) {
