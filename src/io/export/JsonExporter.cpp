...
void JsonExporter::output(std::ostream& out, const fem::Model& model) {
  ...
  for (std::vector<fem::Material>::const_iterator material =
           material_list.begin();
       material != material_list.end(); material++) {
    ...
    out << "\t{\"type\":"
        << "\"linear elastic\", \"label\": \"" << material->getLabel()
  ...
  for (std::map<fem::node_ref_t, fem::Node>::const_iterator it =
           node_list.begin();
       it != node_list.end(); it++) {
    ...
    out << "\n\t";
    out << "\t[\t" << it->first << ",[\t" << it->second.data[0] << ",\t"
        << it->second.data[1] << ",\t" << it->second.data[2] << "\t]]";
  }
  out << "\n\t],\n";
  ...
  if (!model.node_restrictions_list.empty()) {
    ...
    for (std::map<fem::node_restriction_ref_t,
                  fem::NodeRestrictions>::const_iterator it =
             node_restrictions_list.begin();
  ...
  if (!model.load_pattern_list.empty()) {
    ...
    out << "\t\"load patterns\":[";
    for (std::vector<fem::LoadPattern>::const_iterator it =
      ...
      if (!it->nodal_loads.empty()) {
        out << ",\n\t\t\t";
        out << "\"nodal loads\":[\n";
        for (std::map<size_t, fem::NodalLoad>::const_iterator n =
                 it->nodal_loads.begin();
             n != it->nodal_loads.end(); n++) {
          ...
          out << ", \"force\":"
              << "[" << n->second.getForce().x() << ", "
              << n->second.getForce().y() << ", " << n->second.getForce().z()
              << "]}";
        }
        out << "\n\t\t\t]\n";
      }
      ...
      if (!it->domain_loads.empty()) {
        out << ",\n\t\t\t";
        out << "\"domain loads\":[";
        for (std::map<size_t, fem::DomainLoad>::const_iterator n =
                 it->domain_loads.begin();
             n != it->domain_loads.end(); n++) {
          if (n != it->domain_loads.begin())
            out << ",";
          out << "\n\t\t\t\t{";
          out << "\"element\":" << n->first;
          out << ", ";
          out << "\"force\": [" << n->second.force.x() << ","
              << n->second.force.y() << "," << n->second.force.z() << "]";
          out << "}";
        }
        out << "\n\t\t\t]\n";
      }
      ...
      if (!it->surface_loads.empty()) {
        out << ",\n\t\t\t";
        out << "\"surface loads\":[";
        for (std::vector<fem::SurfaceLoad*>::const_iterator surface_load_iter =
                 it->surface_loads.begin();
             surface_load_iter != it->surface_loads.end();
             surface_load_iter++) {
          ...
          for (std::vector<size_t>::const_iterator i =
                   surface_load_nodes.begin();
               i != surface_load_nodes.end(); i++) {
          ...
          for (std::vector<fem::Point3D>::const_iterator i =
                   surface_load->surface_forces.begin();
               i != surface_load->surface_forces.end(); i++) {
