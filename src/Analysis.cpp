...
enum Analysis<Scalar>::Error Analysis<Scalar>::buildEquation(
    Model& model,
    const LoadPattern& lp,
...
template <typename Scalar>
enum Analysis<Scalar>::Error Analysis<Scalar>::generateGlobalStiffnessMatrix(
    Model& model,
    AnalysisResult& result,
    ProgressIndicatorStrategy& progress) {
  ...
  for (Element el : model.getElementList()) {
    std::unique_ptr<BaseElement> element(
        getElement(el));  // TODO remove after migration to polymorphic elements
...
template <typename Scalar>
enum Analysis<Scalar>::Error Analysis<Scalar>::generateGlobalDomainForceVector(
    Model& model,
    const LoadPattern& lp,
    AnalysisResult& result,
    ProgressIndicatorStrategy& progress) {
  ...
  for (auto domain_load_pair : lp.domain_loads) {
    ...
    for (auto quadrature_point : element->getDomainQuadratureRule()) {
      ...
      if (detJ <= 0) {
        ...
        std::cerr << "stumbled on a negative determinant on element_reference "
                  << domain_load_pair.first << std::endl;
    ...
    for (size_t i = 0; i < el.getNodeAmount(); i++) {
      std::map<size_t, boost::tuple<size_t, size_t, size_t>>::iterator
          dof;  // for the force vector scatter operation
      dof = result.lm.find(
          model.getElementByIndex(domain_load_pair.first).getNode(i));
...
template <typename Scalar>
enum Analysis<Scalar>::Error Analysis<Scalar>::generateGlobalSurfaceForceVector(
    Model& model,
    const LoadPattern& lp,
    AnalysisResult& result,
    ProgressIndicatorStrategy& progress) {
...
template <typename Scalar>
enum Analysis<Scalar>::Error Analysis<Scalar>::generateGlobalPointForceVector(
    Model&,
    const LoadPattern& lp,
    AnalysisResult& result,
    ProgressIndicatorStrategy& progress) {
  ...
  for (std::map<size_t, fem::NodalLoad>::const_iterator nodal_load =
           lp.nodal_loads.begin();
       nodal_load != lp.nodal_loads.end(); nodal_load++) {
...
template <typename Scalar>
std::map<size_t, Node> Analysis<Scalar>::displacementsMap(
    AnalysisResult& result) {
  using namespace std;
  ...
  for (map<size_t, boost::tuple<size_t, size_t, size_t>>::const_iterator i =
           result.lm.begin();
       i != result.lm.end(); i++) {
    Node node;

    // don't add a map if no DoF exists
    if ((i->second.get<0>() == 0) && (i->second.get<1>() == 0) &&
        (i->second.get<2>() == 0))
      continue;
...
template <typename Scalar>
void Analysis<Scalar>::generateDisplacementsMap(Model& model,
                                                AnalysisResult& result) {
  ...
  for (auto node_pair : model.getNodeMap()) {
    ...
    d.data[0] = (references.get<0>() == 0)
                    ? 0
                    : result.equation.d(references.get<0>() - 1);
    d.data[1] = (references.get<1>() == 0)
                    ? 0
                    : result.equation.d(references.get<1>() - 1);
    d.data[2] = (references.get<2>() == 0)
                    ? 0
                    : result.equation.d(references.get<2>() - 1);
...
template <typename Scalar>
enum Analysis<Scalar>::Error Analysis<Scalar>::recoverValues(
    Model& model,
    AnalysisResult& result) {
...
template <typename Scalar>
enum Analysis<Scalar>::Error Analysis<Scalar>::calculateStrainEnergy(
    Model&,
    AnalysisResult& result) {
...
template <typename Scalar>
void Analysis<Scalar>::makeLocationMatrix(Model& model,
                                          AnalysisResult& result) {
  size_t dof = 1;  // degree of freedom count, the 0 value is interpreted as a
                   // prescribed movement
  ...
  for (auto node_pair : model.getNodeMap()) {
    size_t node_id = 0;
    std::tie(node_id, std::ignore) = node_pair;
    if (model.node_restrictions_list.find(node_id) ==
        model.node_restrictions_list.end()) {
      ...
      NodeRestrictions node_restriction =
          model.getNodeRestrictionsByIndex(node_id);
...
template <typename Scalar>
inline void Analysis<Scalar>::addElementaryStiffnessToGlobal(
    const Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>& k_elem,
  ...
  std::map<size_t, boost::tuple<size_t, size_t, size_t>>::iterator idof,
      jdof;  // degrees of freedom for the line and column

  for (int i = 0; i < element_node_amount; i++) {
    // if all three DoF of this node are prescribed then they aren't added to
    // K_g
    ...
    // get the global DoF
    size_t id[3];  // boost::tuple, being a template, doesn't accept non-const
                   // parameters.  This is a way to sidestep it
    ...
    for (int j = 0; j < element_node_amount; j++) {
      ...
      // get the global DoF
      size_t jd[3];  // boost::tuple, being a template, doesn't accept non-const
                     // parameters.  This is a way to sidestep it
      ...
      for (int u = 0; u < 3; u++) {
        ...
        for (int v = 0; v < 3; v++) {
          if ((id[u] != 0) && (jd[v] != 0)) {
            result.equation.K(id[u] - 1, jd[v] - 1) +=
                k_elem(3 * i + u, 3 * j + v);
          }
        }
      }
