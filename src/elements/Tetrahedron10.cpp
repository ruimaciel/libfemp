...
Tetrahedron10::Tetrahedron10() : TetrahedronFamily() {
  ...
  m_stiffness_quadrature_rule =
      std::unique_ptr<Tetrahedron4>(new Tetrahedron4());

  m_domain_quadrature_rule =
      std::unique_ptr<Tetrahedron14>(new Tetrahedron14());
}

std::vector<double> Tetrahedron10::getN(const Point3D& p) {
...
std::vector<double> Tetrahedron10::getdNdcsi(const Point3D& p) {
  double csi = p.x();
  double eta = p.y();
  double zeta = p.z();
...
std::vector<double> Tetrahedron10::getdNdeta(const Point3D& p) {
  double csi = p.x();
  double eta = p.y();
  double zeta = p.z();
...
std::vector<double> Tetrahedron10::getdNdzeta(const Point3D& p) {
  double csi = p.x();
  double eta = p.y();
  double zeta = p.z();
