...
template <typename Scalar>
void LinearAnalysis<Scalar>::set(Model& model,
                                 LoadPattern& lp,
...
template <typename Scalar>
enum Analysis<Scalar>::Error LinearAnalysis<Scalar>::run(
    Model& model,
    LoadPattern& lp,
    AnalysisResult& result,
    ProgressIndicatorStrategy& progress) {
  ...
  if ((error = this->buildEquation(model, lp, result, progress)) !=
      Analysis<Scalar>::ERR_OK) {
