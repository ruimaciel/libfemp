...
Vector<scalar, DenseVector> operator*(Matrix<scalar, MatrixStoragePolicy>& m,
...
Vector<scalar, DenseVector> operator*(Matrix<scalar, SparseCRS>& m,
                                      Vector<scalar, VectorStoragePolicy>& v) {
  using namespace std;
  ...
  for (size_t row = 0; row != m.rows(); row++) {
    v_out(row) = 0;
    for (size_t i = m.data.row_pointer[row]; i < m.data.row_pointer[row + 1];
         i++) {
      // cout << "i:" << i << endl;
