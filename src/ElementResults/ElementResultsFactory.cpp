...
ElementResultsFactory::ElementResultsFactory(fem::Model& model,
                                             AnalysisResult& results) {
  this->m_model = &model;
  this->m_analysis_result = &results;
}

ElementResults* ElementResultsFactory::operator()(
    const fem::Element& reference_element) {
  ...
  for (unsigned int coord = 0; coord < coordinates.size(); coord++) {
    fem::Point3D local =
        coordinates[coord];  // Point in element in \xi, the local coordinates
    ...
    for (unsigned int node = 0; node < coordinates.size(); node++) {
      // get the node's x coordinate, the coordinate in the global frame of
      // reference
    ...
    for (unsigned int node = 0; node < coordinates.size(); node++) {
      ...
      d = this->m_analysis_result->displacements[reference_element.getNode(
          node)];  // displacements calculated in this node

      // calculate \epsilon_{11} = dNdx_1*d1
      dNdx = invDg(0, 0) * dNdcsi[node] + invDg(0, 1) * dNdeta[node] +
             invDg(0, 2) * dNdzeta[node];
      dNdy = invDg(1, 0) * dNdcsi[node] + invDg(1, 1) * dNdeta[node] +
             invDg(1, 2) * dNdzeta[node];
      dNdz = invDg(2, 0) * dNdcsi[node] + invDg(2, 1) * dNdeta[node] +
             invDg(2, 2) * dNdzeta[node];
    ...
    // set the stresses
    Material material =
        m_model->getMaterialByIndex(reference_element.getMaterialRef());
    const double E = material.getYoungsModulus();
    const double nu = material.getPoissonRatio();

    results->stresses[coord].s11 =
        ((1 - nu) * results->strains[coord].e11 +
         nu * results->strains[coord].e22 + nu * results->strains[coord].e33) *
        E / ((1 + nu) * (1 - 2 * nu));
    results->stresses[coord].s22 = (nu * results->strains[coord].e11 +
                                    (1 - nu) * results->strains[coord].e22 +
                                    nu * results->strains[coord].e33) *
                                   E / ((1 + nu) * (1 - 2 * nu));
    results->stresses[coord].s33 =
        (nu * results->strains[coord].e11 + nu * results->strains[coord].e22 +
         (1 - nu) * results->strains[coord].e33) *
        E / ((1 + nu) * (1 - 2 * nu));
    results->stresses[coord].s12 =
        0.5 * results->strains[coord].e12 * E / (1 + nu);
    results->stresses[coord].s23 =
        0.5 * results->strains[coord].e23 * E / (1 + nu);
    results->stresses[coord].s13 =
        0.5 * results->strains[coord].e13 * E / (1 + nu);

    // set the von mises
    Stresses<double> s = results->stresses[coord];
    results->von_mises[coord] =
        0.7071067812 *
        sqrt((s.s11 - s.s22) * (s.s11 - s.s22) +
...
// ----------------------------------------------------------------------------
int ElementResultsFactory::dsyevj3(double A[3][3],
                                   double Q[3][3],
                                   double w[3]) const
// ----------------------------------------------------------------------------
