...
Triangle10::Triangle10() {
  ...
  m_domain_quadrature_rule =
      std::unique_ptr<quadrature::Triangle6>(new quadrature::Triangle6());
}

std::vector<double> Triangle10::getN(const Point3D& p) const {
  ...
  N[0] =
      4.5 * (-eta - csi + 1.0 / 3) * (-eta - csi + 2.0 / 3) * (-eta - csi + 1);
...
std::vector<double> Triangle10::getdNdcsi(const Point3D& p) const {
  double csi = p.x();
  double eta = p.y();

  std::vector<double> dNdcsi(this->getNodeAmount());

  dNdcsi[0] = -(27 * eta * eta + (54 * csi - 36) * eta + 27 * csi * csi -
                36 * csi + 11) /
              2.0;
  dNdcsi[1] = (27 * csi * csi - 18 * csi + 2) / 2.0;
  dNdcsi[2] = 0;
  dNdcsi[3] = (27 * eta * eta + (108 * csi - 45) * eta + 81 * csi * csi -
               90 * csi + 18) /
              2.0;
...
std::vector<double> Triangle10::getdNdeta(const Point3D& p) const {
  double csi = p.x();
  double eta = p.y();

  std::vector<double> dNdeta(this->getNodeAmount());

  dNdeta[0] = -(27 * eta * eta + (54 * csi - 36) * eta + 27 * csi * csi -
                36 * csi + 11) /
              2.0;
  dNdeta[1] = 0;
  ...
  dNdeta[8] = (81 * eta * eta + (108 * csi - 90) * eta + 27 * csi * csi -
               45 * csi + 18) /
              2.0;
...
std::vector<fem::Point3D> Triangle10::getLocalCoordinates() const {
  return {
      Point3D(0.0, 0.0, 0), Point3D(1.0, 0.0, 0), Point3D(0.0, 1.0, 0),
      Point3D(0.5, 0.0, 0), Point3D(0.5, 0.5, 0), Point3D(0.0, 0.5, 0),
  };
}
