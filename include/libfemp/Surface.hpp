...
class Surface {
 public:
  struct ReferencedElement {
    ...
    unsigned char
        surface;  // the surface numbering.  In a hexahedron it can be [1-6]
    std::vector<size_t>
        element_local_reference;  // a list of local references to the nodes in
 ...
 public:
  ...
  void pushElementReference(const size_t reference,
                            const Element::Type& element_type,
                            const unsigned char surface);

  /**
   * @return		true if it's an internal surface, false if it's an
   * external surface
   */
  bool internal();
  bool external();
   ...
   * @return	true if the number of nodes is compatible with the surface type
   */
  bool compatible(const Element::Type& m_type,
                  std::vector<size_t>& m_node_references);
