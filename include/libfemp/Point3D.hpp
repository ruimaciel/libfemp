...
class Point3D {
 public:
  ...
  void
  normalize();  // converts the vector to it's director vector (norm() == 1)
  ...
  friend Point3D cross_product(const Point3D& LHV,
                               const Point3D& RHV);  // assuming vector is 3D
  friend double dot_product(const Point3D& LHV, const Point3D& RHV);
  friend Point3D getNormalVector(const Point3D& a,
                                 const Point3D& b,
                                 const Point3D& c);
};

Point3D cross_product(const Point3D& LHV,
                      const Point3D& RHV);  // assuming vector is 3D
