...
void SurfaceNormalLoad::operator()(fem::SurfaceLoad& surface_load,
                                   fem::Model& model) {
  fem::SurfaceLoad* load =
      nullptr;  // used to store a pointer to the current object: nasty hack due
  ...
  switch (surface_load.getType()) {
    ...
    default:  // if this part was reached then something is broken
      std::cerr << "SurfaceNormalLoad(): invalid surface load type"
                << std::endl;
      return;
      break;
  }
