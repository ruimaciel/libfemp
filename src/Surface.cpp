...
void Surface::pushElementReference(const size_t reference,
    ...
    case Element::FE_PRISM6:
      ...
      switch (surface) {
        ...
        case 3:
          /*
                                                                     prism
             faces: 3 quadrangles, 2 triangles
                                                                   */
...
bool Surface::compatible(const Element::Type& type,
...
bool Surface::operator==(const Surface& other) const {
  ...
  for (i = this->m_node_references.begin(); i != this->m_node_references.end();
       i++) {
    for (j = other.m_node_references.begin();
         j != other.m_node_references.end(); j++) {
      if (*j == *i)
        break;
    }
