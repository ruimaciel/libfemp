...
scalar& SparseCRS<scalar>::operator()(const size_t row, const size_t column) {
  ...
  // the first element in this row has a greater column index than the one being
  // referenced
...
void SparseCRS<scalar>::dump() {
  ...
  cout << "col_ind:";
  for (vector<size_t>::iterator i = data.column_index.begin();
       i != data.column_index.end(); i++) {
    cout << " " << *i;
  }
  cout << endl;

  cout << "row_ptr:";
  for (vector<size_t>::iterator i = data.row_pointer.begin();
       i != data.row_pointer.end(); i++) {
    cout << " " << *i;
  }
  cout << "\n" << endl;
  //*/
