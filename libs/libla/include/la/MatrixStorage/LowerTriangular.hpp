...
template <typename scalar>
LowerTriangular<scalar>::LowerTriangular(const size_t rows,
...
void LowerTriangular<scalar>::clear() {
  using namespace std;
  for (typename vector<scalar>::iterator i = data.data.begin();
       i != data.data.end(); i++) {
    *i = 0;
  }
}
...
template <typename scalar>
scalar& LowerTriangular<scalar>::operator()(const size_t row,
                                            const size_t column) {
  assert(column < data.t_columns);
  assert(row >= column);
