...
enum FemJsonParser::Error::Type FemJsonParser::parse(std::istream& file,
  ...
  while (!ss.empty()) {
    if (terminal_token == ss.top()) {
      ...
      switch (
          table[(enum NonTerminalTokens)nonterminal_token][terminal_token]) {
        case PR_LABEL:
        ...
        case PR_REFERENCE_ERROR:
          this->error.code =
              Error::Type::ERR_UNKNOWN;  // TODO return a dedicated error
          this->error.message = "Expected a reference number at line " +
        ...
        case PR_NODE_GROUP_SET: {
          fem::NodeGroup n;
          n.setLabel(label);
          for (std::vector<size_t>::const_iterator i = reference.begin();
               i != reference.end(); i++) {
            if (*i < model.getNodeMap().size())
              n.pushNode(*i);
            else {
              // TODO warning message
              std::cerr << "FemJsonParser: node group " << label
                        << " refers to invalid node: " << *i << std::endl;
            }
          }
          reference.clear();
        ...
        case PR_NODE_RESTRICTION_SET:
          // TODO finish this
          for (std::vector<size_t>::iterator i = reference.begin();
               i != reference.end(); i++) {
        ...
        case PR_DOMAIN_LOAD_SET:
          for (std::vector<size_t>::iterator i = reference.begin();
            ...
            if (*i < model.numberOfElements()) {
               ...
               * doesn't exist */
              std::cerr
                  << "Error: domain load applied to non-existing element #"
                  << *i << std::endl;
            }
          }
          reference.clear();
        ...
        default:
          ...
          this->error.message = "Unknown error while parsing line " +
                                boost::lexical_cast<std::string>(line_number);
          return this->error.code;
          break;
      }
